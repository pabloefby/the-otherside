...
function Brujeria() {
  ...
  const [alertText, setAlertText] = useState("");
    ...
    try {
      const resp = await axios.get(`http://localhost:3001/publis-category-trend/${"Brujeria"}`);
  ...
  const getPublis = async () => {
    try {
       const resp = await axios.get(`http://localhost:3001/publis-category/${"Brujeria"}`);
      if (resp.data.msg === "Error BD") {
        alert("Error con la BD");
      } else if (resp.data.msg === "Vacio") {
        setAlertText("Aun no hay publicaciones");
